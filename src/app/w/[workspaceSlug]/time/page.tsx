...
import { humanStatus, classForStatus, formatDateTime, formatMoney, formatMinutes, relativeTime } from "@/lib/format";
...
export default async function TimePage({
  ...
  const [entries, projects, allEntries] = await Promise.all([
    ...
    db.project.findMany({ where: { workspaceId: ctx.workspaceId }, select: { id: true, name: true, clientId: true, client: { select: { name: true } } } }),
  ...
  const revenue = entries.filter((e) => e.billable).reduce((s, e) => s + e.rateMinor * BigInt(e.minutes) / 60n, 0n);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
              ...
              <tbody>
                {entries.map((e) => (
                  <tr key={e.id} className="border-b border-border/40 last:border-0 hover:bg-muted/30">
                    ...
                    <td className="px-4 py-2.5 text-muted-foreground">{e.project?.name ?? e.client?.name ?? "—"}</td>
                    <td className="px-4 py-2.5 text-muted-foreground">{relativeTime(e.startedAt)}</td>
                    <td className="px-4 py-2.5 text-right tabular-nums">{formatMinutes(e.minutes)}</td>
                    ...
                    {can(ctx, "time.manage_own") && (
                      <td className="px-4 py-2.5 text-right">
                        {e.status === "open" && (
                          <SubmitTimeButton workspaceSlug={workspaceSlug} ids={[e.id]} label="Submit" />
                        )}
                      </td>
                    )}
                  </tr>
      ...
      {can(ctx, "time.read_all") && allEntries.length > 0 && (
        <div className="mt-8">
          <h2 className="mb-3 text-sm font-medium uppercase tracking-wider text-muted-foreground">All team time</h2>
          <Card className="overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="border-b border-border/60 bg-muted/30 text-xs uppercase tracking-wider text-muted-foreground">
