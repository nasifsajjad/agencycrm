...
export default async function RolesPage({
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
      <PageHeader title="Roles & permissions" description={`${roles.length} roles · ${PERMISSIONS.length} permissions in catalogue`} />
            ...
            <Card key={r.id}>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-1">
                  {r.permissions.map((p) => (
                    <code key={p.permission.key} className="rounded bg-muted/60 px-1.5 py-0.5 text-[10px]">{p.permission.key}</code>
                  ))}
                  {r.permissions.length === 0 && <span className="text-xs text-muted-foreground">No permissions assigned.</span>}
                </div>
              </CardContent>
            </Card>
          ))}
