...
export default async function DealsPage({
  ...
  const [pipeline, deals, companies, contacts] = await Promise.all([
    db.pipeline.findFirstOrThrow({ where: { workspaceId: ctx.workspaceId, isDefault: true }, include: { stages: { orderBy: { position: "asc" } } } }),
    db.deal.findMany({
    ...
    db.company.findMany({ where: { workspaceId: ctx.workspaceId }, select: { id: true, name: true }, orderBy: { name: "asc" } }),
    db.contact.findMany({ where: { workspaceId: ctx.workspaceId }, select: { id: true, firstName: true, lastName: true, email: true }, orderBy: { lastName: "asc" } }),
  ]);

  const totalValue = deals.reduce((s, d) => s + d.amountMinor, 0n);
  const weightedValue = deals.reduce((s, d) => s + (d.amountMinor * BigInt(d.probability)) / 100n, 0n);
    ...
    <div className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">
      ...
      {deals.length === 0 ? (
        <EmptyState
          ...
          action={can(ctx, "crm.create") ? { label: "New deal", href: `/w/${workspaceSlug}/crm/deals?new=1` } : undefined}
        />
      ) : (
        <DealsBoard
          workspaceSlug={workspaceSlug}
