    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      {leads.length === 0 ? (
        <EmptyState
          ...
          action={can(ctx, "crm.create") ? { label: "New lead", href: `/w/${workspaceSlug}/crm/leads?new=1` } : undefined}
        />
      ) : (
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="truncate text-sm font-medium">
                          {l.contact ? `${l.contact.firstName} ${l.contact.lastName}` : l.company?.name ?? "Unnamed"}
                        </div>
                        {l.contact?.email && (
                          <div className="truncate text-xs text-muted-foreground">{l.contact.email}</div>
                        )}
                        {l.company && (
                          <Link href={`/w/${workspaceSlug}/crm/companies/${l.company.id}`} className="text-xs text-muted-foreground hover:underline">
                            {l.company.name}
                          </Link>
                        )}
                      </div>
                      <Badge variant="outline" className={l.score >= 70 ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" : "text-muted-foreground"}>
                        {l.score}
                      </Badge>
                    </div>
                    <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      {l.owner && (
                        <Avatar className="h-5 w-5">
                          <AvatarFallback className="text-[9px]">{initials(l.owner.displayName)}</AvatarFallback>
                        </Avatar>
                      )}
                    </div>
                    {can(ctx, "crm.update") && (
                      <div className="mt-2">
                        <LeadStatusSelect workspaceSlug={workspaceSlug} leadId={l.id} currentStatus={l.status} />
                      </div>
                    )}
                  </Card>
                ))}
