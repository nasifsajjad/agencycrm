    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      {contacts.length === 0 ? (
        <EmptyState
          ...
          description={q ? "Try a different query." : "Add your first contact to start building your CRM."}
          action={!q && can(ctx, "crm.create") ? { label: "New contact", href: `/w/${workspaceSlug}/crm/contacts?new=1` } : undefined}
        />
      ) : (
                ...
                {contacts.map((c) => (
                  <tr key={c.id} className="border-b border-border/40 last:border-0 hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-2.5">
                      <Link href={`/w/${workspaceSlug}/crm/contacts/${c.id}`} className="flex items-center gap-2">
