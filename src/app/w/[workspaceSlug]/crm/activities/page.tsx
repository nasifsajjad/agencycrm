...
import { CheckCircle2, Circle, Phone, Mail, CalendarClock, StickyNote, ListTodo } from "lucide-react";
          ...
          {activities.map((a) => {
            ...
            return (
              <div key={a.id} className="flex items-center gap-3 px-4 py-3">
                <div className={`grid h-8 w-8 place-items-center rounded-md ${a.completedAt ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" : "bg-foreground/5 text-foreground"}`}>
                  <Icon className="h-4 w-4" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`truncate text-sm font-medium ${a.completedAt ? "line-through text-muted-foreground" : ""}`}>
                      {a.subject ?? a.body?.slice(0, 80) ?? "Untitled activity"}
                    </span>
                    <Badge variant="outline" className="capitalize">{a.activityType}</Badge>
                    {isOverdue && <Badge variant="outline" className="bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300">Overdue</Badge>}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {a.contact && <span>{a.contact.firstName} {a.contact.lastName} · </span>}
                    {a.deal && <span>{a.deal.name} · </span>}
                    {a.dueAt && <span>Due {formatDateTime(a.dueAt)} ({relativeTime(a.dueAt)})</span>}
