...
export default async function PortalApprovalsPage({ params }: { params: Promise<{ portalSlug: string }> }) {
  ...
  return (
    <div>
      ...
      {approvals.length === 0 ? (
        <EmptyState title="No approvals yet" description="When your team requests approval on a deliverable, it will appear here." />
      ) : (
            ...
            <Card key={a.id} className="p-4">
              <div className="flex items-start justify-between">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 text-xs text-muted-foreground">Version {a.versionNumber} · Requested {relativeTime(a.createdAt)}</div>
                  {a.instructions && <p className="mt-2 text-sm text-muted-foreground">{a.instructions}</p>}
                  {a.dueAt && <div className="mt-2 text-xs text-muted-foreground">Due {formatDate(a.dueAt)}</div>}
                </div>
                <Badge variant="outline" className={classForStatus(a.status)}>{humanStatus(a.status)}</Badge>
              </div>
              {a.status === "pending" && (
                <div className="mt-4 border-t border-border/40 pt-3">
                  <PortalApprovalDecision workspaceSlug={portal.workspace.slug} approvalId={a.id} portalSlug={portalSlug} />
                </div>
              )}
            </Card>
          ))}
