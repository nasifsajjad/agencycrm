...
export default async function PortalReportsPage({ params }: { params: Promise<{ portalSlug: string }> }) {
      ...
      <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Card>
          <CardContent className="p-4">
            ...
            <div className="mt-1 text-xl font-semibold tabular-nums">{formatMoney(totalInvoiced)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
          ...
          <CardContent>
            {invoices.length === 0 ? (
              ...
              <div className="space-y-2 text-sm">
                {invoices.map((i) => (
                  <div key={i.id} className="flex items-center justify-between border-b border-border/30 py-1.5 last:border-0">
                    <div>
              ...
              <div className="space-y-2 text-sm">
                {retainers.map((r) => (
                  <div key={r.id} className="border-b border-border/30 py-1.5 last:border-0">
                    ...
                    <div className="text-xs text-muted-foreground">{formatMoney(r.amountMinor)}/mo · {Math.floor(r.includedMinutes / 60)}h included</div>
                  </div>
                ))}
              </div>
            )}
