...
export function DealsBoard({
  ...
  stages: { id: string; name: string; color?: string | null; isClosed?: boolean; isWon?: boolean; probability?: number }[];
  deals: Deal[];
  canEdit: boolean;
}) {
  ...
  async function onDragEnd(result: DropResult) {
    ...
    const dealId = String(result.draggableId);
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="flex gap-3 overflow-x-auto pb-4">
        {stages.map((stage) => {
          ...
          return (
            <div key={stage.id} className="flex w-72 shrink-0 flex-col rounded-lg border border-border/60 bg-muted/20">
              <div className="flex items-center justify-between border-b border-border/60 px-3 py-2">
                <div className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: stage.color ?? "var(--muted-foreground)" }} />
                  <span className="text-sm font-medium">{stage.name}</span>
                  <span className="text-xs text-muted-foreground">{stageDeals.length}</span>
                </div>
                <span className="text-xs text-muted-foreground tabular-nums">{formatMoney(total)}</span>
              </div>
...
function DraggableDealCard({ deal, pending, disabled }: { deal: Deal; pending: boolean; disabled: boolean }) {
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          ...
          {deal.company && <div className="truncate text-xs text-muted-foreground">{deal.company}</div>}
        </div>
        <div className="text-right">
          <div className="text-sm font-medium tabular-nums">{formatMoney(BigInt(deal.amountMinor))}</div>
