...
async function withPermission<T>(slug: string, perm: Permission, fn: () => Promise<T>): Promise<{ ok?: true; data?: T; error?: string }> {
...
export async function createContactAction(slug: string, formData: FormData) {
  return withPermission(slug, "crm.create", async () => {
    ...
    await audit({ ctx, action: "contact.created", entityType: "contact", entityId: contact.id, after: { name: `${firstName} ${lastName}`, email } });
...
export async function deleteContactAction(slug: string, id: string) {
  return withPermission(slug, "crm.delete", async () => {
    ...
    await audit({ ctx, action: "contact.deleted", entityType: "contact", entityId: id, before: { name: `${contact.firstName} ${contact.lastName}` } });
    revalidatePath(`/w/${slug}/crm/contacts`);
    return true;
  });
}
...
export async function createCompanyAction(slug: string, formData: FormData) {
  return withPermission(slug, "crm.create", async () => {
    ...
    await audit({ ctx, action: "company.created", entityType: "company", entityId: company.id, after: { name, domain } });
...
export async function createLeadAction(slug: string, formData: FormData) {
  return withPermission(slug, "crm.create", async () => {
    ...
    if (company) {
      const existing = await db.company.findFirst({ where: { workspaceId: ctx.workspaceId, name: company } });
      if (existing) companyId = existing.id;
      else {
        const c = await db.company.create({ data: { workspaceId: ctx.workspaceId, name: company, ownerId: ctx.userId, lifecycleStage: "lead" } });
        companyId = c.id;
      }
    }
    ...
    await audit({ ctx, action: "lead.created", entityType: "lead", entityId: lead.id, after: { name: `${firstName} ${lastName}`, email, source, score } });
...
export async function updateLeadStatusAction(slug: string, id: string, status: string) {
  return withPermission(slug, "crm.update", async () => {
    ...
    const updated = await db.lead.update({ where: { id }, data: { status, qualifiedAt: status === "qualified" ? new Date() : lead.qualifiedAt } });
    await audit({ ctx, action: "lead.status_changed", entityType: "lead", entityId: id, before: { status: lead.status }, after: { status } });
    revalidatePath(`/w/${slug}/crm/leads`);
    return updated;
  });
}
...
export async function createDealAction(slug: string, formData: FormData) {
  return withPermission(slug, "crm.create", async () => {
    ...
    const ctx = await resolveWorkspace(slug);
    const pipeline = await db.pipeline.findFirstOrThrow({ where: { workspaceId: ctx.workspaceId, isDefault: true } });
    const stage = stageId ? await db.pipelineStage.findFirst({ where: { id: stageId, pipelineId: pipeline.id } }) : await db.pipelineStage.findFirst({ where: { pipelineId: pipeline.id, position: 0 } });
    ...
    await audit({ ctx, action: "deal.created", entityType: "deal", entityId: deal.id, after: { name, amount } });
...
export async function createActivityAction(slug: string, formData: FormData) {
  return withPermission(slug, "crm.create", async () => {
    ...
    await audit({ ctx, action: "activity.created", entityType: "activity", entityId: activity.id, after: { activityType, subject, entityType, entityId } });
...
export async function createNoteAction(slug: string, formData: FormData) {
  return withPermission(slug, "comments.create", async () => {
    ...
    const note = await db.note.create({
      data: { workspaceId: ctx.workspaceId, bodyRich, entityType, entityId, visibility, authorId: ctx.userId },
    });
    await audit({ ctx, action: "note.created", entityType: "note", entityId: note.id, after: { entityType, entityId, visibility } });
    revalidatePath(`/w/${slug}/crm/contacts`);
...
export async function createClientAction(slug: string, formData: FormData) {
  return withPermission(slug, "clients.create", async () => {
    ...
    await audit({ ctx, action: "client.created", entityType: "client", entityId: client.id, after: { name, code, status } });
    revalidatePath(`/w/${slug}/clients`);
    return client;
  });
}
...
export async function createProjectAction(slug: string, formData: FormData) {
  return withPermission(slug, "projects.create", async () => {
    ...
    const ctx = await resolveWorkspace(slug);
    const status = await db.projectStatus.findFirst({ where: { workspaceId: ctx.workspaceId, category: "planning" } });
    ...
    await audit({ ctx, action: "project.created", entityType: "project", entityId: project.id, after: { name, code, clientId } });
    revalidatePath(`/w/${slug}/projects`);
    return project;
  });
}
...
export async function createTaskAction(slug: string, formData: FormData) {
  return withPermission(slug, "tasks.create", async () => {
    ...
    await audit({ ctx, action: "task.created", entityType: "task", entityId: task.id, after: { name, projectId, assigneeId } });
...
export async function updateTaskStatusAction(slug: string, taskId: string, statusId: string) {
  return withPermission(slug, "tasks.update", async () => {
    ...
    const status = await db.taskStatus.findFirst({ where: { id: statusId, workspaceId: ctx.workspaceId } });
...
export async function assignTaskAction(slug: string, taskId: string, assigneeId: string | null) {
  return withPermission(slug, "tasks.assign", async () => {
    ...
    await audit({ ctx, action: "task.assigned", entityType: "task", entityId: taskId, after: { assigneeId } });
    revalidatePath(`/w/${slug}/tasks`);
    revalidatePath(`/w/${slug}/my-work`);
    return updated;
  });
...
export async function decideApprovalAction(slug: string, approvalId: string, decision: "approved" | "changes_requested", note: string) {
  return withPermission(slug, "approvals.decide", async () => {
    ...
    await db.$transaction(async (tx) => {
      ...
      await tx.approvalEvent.create({
        data: { approvalRequestId: approval.id, actorUserId: ctx.userId, action: decision, note: note || null },
      });
      // If there's another pending step after this one, leave the overall status pending.
      const remainingSteps = approval.steps.filter((s) => s.id !== step.id && s.status === "pending");
    ...
    await audit({ ctx, action: `approval.${decision}`, entityType: "approval", entityId: approvalId, after: { note } });
...
export async function createTimeEntryAction(slug: string, formData: FormData) {
  return withPermission(slug, "time.manage_own", async () => {
    ...
    const ctx = await resolveWorkspace(slug);
    const project = projectId ? await db.project.findFirst({ where: { id: projectId, workspaceId: ctx.workspaceId }, include: { client: true } }) : null;
    ...
    await audit({ ctx, action: "time_entry.created", entityType: "time_entry", entityId: entry.id, after: { minutes, projectId, billable } });
...
export async function inviteMemberAction(slug: string, formData: FormData) {
  return withPermission(slug, "members.invite", async () => {
    ...
    const ctx = await resolveWorkspace(slug);
    const role = await db.role.findUnique({ where: { workspaceId_name: { workspaceId: ctx.workspaceId, name: roleName } } });
    ...
    await audit({ ctx, action: "invitation.created", entityType: "invitation", entityId: invitation.id, after: { email, role: roleName } });
...
export async function removeMemberAction(slug: string, membershipId: string) {
  return withPermission(slug, "members.remove", async () => {
    ...
    await db.workspaceMembership.update({ where: { id: membershipId }, data: { status: "removed" } });
