...
async function executeAction(event: any, action: any) {
  ...
  switch (action.actionType) {
    ...
    case "email": {
      // Local capture adapter: log to console + store in outbox payload
      // Production would call the email adapter
      console.log(`[email-capture] To: ${config.to} Subject: ${config.subject}`)
      break
    }
    case "task": {
    ...
    case "webhook": {
      // Outbound webhook delivery — would be queued via webhook_deliveries
      // For local mode, just log
      console.log(`[webhook] Would deliver to: ${config.url}`)
      break
    }
    default:
      // Unknown action — skip
