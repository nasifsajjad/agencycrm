...
describe("automation condition evaluation", () => {
  ...
  it("distinguishes missing, null, and false values", () => {
    expect(evaluateConditionTree({ field: "payload.missing", operator: "not_exists" }, event)).toBe(
      true
    )
    expect(
      evaluateConditionTree(
        { field: "payload.stage", operator: "not_equals", value: "lost" },
        event
      )
    ).toBe(true)
    expect(
      evaluateConditionTree(
        { not: { field: "payload.stage", operator: "equals", value: "won" } },
        event
      )
    ).toBe(false)
  })
  ...
  it("rejects unknown operators instead of treating them as a match", () => {
    expect(() =>
      evaluateConditionTree({ field: "stage", operator: "regex", value: "won" }, event)
    ).toThrow("Unsupported")
  })
})
