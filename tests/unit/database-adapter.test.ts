...
class FakeBuilder implements PromiseLike<{
  ...
  private async execute() {
    ...
    if (
      this.selected
        .split(",")
        .some((column) => column !== "*" && !this.client.columns[this.table]?.has(column))
    ) {
    ...
    rows = rows.filter((row) =>
      this.filters.every((filter) => {
        ...
        if (filter.kind === "not" && filter.operator === "in")
          return !(filter.value as string).includes(JSON.stringify(actual))
    ...
    if (this.operation === "update") {
      ...
      this.client.rows[this.table] = this.client.rows[this.table].filter(
    ...
    const data = rows.map((row) =>
      this.selected === "*"
        ? { ...row }
        : Object.fromEntries(this.selected.split(",").map((column) => [column, row[column]]))
    )
...
describe("Supabase database adapter semantics", () => {
  ...
  it("rejects relation predicates it cannot faithfully express", async () => {
    ...
    await expect(
      db.company.findMany({ where: { contacts: { every: { email: { contains: "@" } } } } })
  ...
  it("makes empty in filters empty and notIn filters unconstrained", async () => {
    ...
    expect(
      await db.company.findMany({ where: { id: { notIn: [] } }, select: { id: true } })
    ).toHaveLength(2)
  })
  ...
  it("preflights single-row updates so a non-unique filter cannot partially mutate", async () => {
    ...
    const db = createDatabase(() => client as never)
    await expect(
      db.company.update({ where: { workspaceId: "workspace-a" }, data: { name: "wrong" } })
