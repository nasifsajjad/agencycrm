...
export default async function TasksPage({
  ...
  const overdue = tasks.filter(
    (t) =>
      t.dueAt &&
      !t.dueAt.toString().startsWith("1970") &&
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      {tasks.length === 0 ? (
        <EmptyState
          ...
          action={
            can(ctx, "tasks.create")
              ? { label: "New task", href: `/w/${workspaceSlug}/tasks?new=1` }
              : undefined
          }
        />
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {tasks.map((t) => {
                  const isOverdue =
                    t.dueAt && new Date(t.dueAt) < new Date() && t.status?.category !== "done"
                  return (
                    <tr
                      key={t.id}
                    ...
                    >
                      <td className="px-4 py-2.5 font-medium">
                        {t.project ? (
                          <Link
                            href={`/w/${workspaceSlug}/projects/${t.project.id}`}
                      ...
                      <td
                        className={`px-4 py-2.5 ${isOverdue ? "text-danger font-medium" : "text-muted-foreground"}`}
                      >
