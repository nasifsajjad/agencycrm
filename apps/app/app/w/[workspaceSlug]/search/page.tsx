...
export default async function SearchPage({
  ...
  if (query) {
    ...
    results = [
      ...contacts.map((c) => ({
        ...
        href: `/w/${workspaceSlug}/crm/contacts`,
        icon: UserCircle,
      })),
      ...companies.map((c) => ({
        type: "Company",
        ...
        href: `/w/${workspaceSlug}/crm/companies`,
        icon: Building2,
      })),
      ...clients.map((c) => ({
        type: "Client",
        ...
        href: `/w/${workspaceSlug}/clients/${c.id}`,
        icon: Briefcase,
      })),
      ...projects.map((p) => ({
        type: "Project",
      ...
      ...deals.map((d) => ({
        ...
        id: d.id,
        label: d.name,
        href: `/w/${workspaceSlug}/crm/deals`,
        icon: KanbanSquare,
      })),
      ...approvals.map((a) => ({
        type: "Approval",
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6 lg:px-8">
      <PageHeader
        title="Search"
        description="Search across contacts, companies, clients, projects, deals, and approvals."
      />
      <form className="mb-6">
      ...
      {!query ? (
        <EmptyState
          ...
          description="Search is permission-aware — you'll only see records you can access."
        />
      ) : results.length === 0 ? (
