...
export async function sendEmail(message: EmailMessage): Promise<{ providerMessageId: string }> {
  ...
  try {
    ...
    if (typeof parsed.id !== "string" || !parsed.id) throw new Error("Email provider returned no message id")
...
export async function sendWebhook(url: string, payload: Record<string, unknown>, secret: string): Promise<number> {
  let parsed: URL
  try { parsed = new URL(url) } catch { throw new Error("Webhook URL is invalid") }
  if (parsed.protocol !== "https:" && !(process.env.NODE_ENV !== "production" && process.env.ALLOW_INSECURE_WEBHOOKS === "true")) {
    throw new Error("Webhook URL must use HTTPS")
  }
  if (parsed.username || parsed.password) throw new Error("Webhook URL cannot contain credentials")
  const addresses = await dns.lookup(parsed.hostname, { all: true })
  if (addresses.some(({ address }) => privateAddress(address))) throw new Error("Webhook URL resolves to a private address")
  ...
  try {
    const response = await fetch(parsed, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-AgencyOS-Signature": `sha256=${signature}` },
      body,
      signal: controller.signal,
    })
