...
async function run(model: string, operation: string, args: QueryArgs = {}): Promise<unknown> {
  ...
  if (operation === "upsert") {
    ...
    const result = await tableClient
      .upsert(toSql(model, args.data), { onConflict: conflict })
      .select("*")
    if (result.error) throw result.error
    return toModel(model, result.data?.[0] ?? null)
  }
...
export const db = new Proxy(
  database as Record<string, Delegate> & {
    $transaction<T>(callback: (tx: typeof db) => Promise<T>): Promise<T>
  },
  {
    get(target, property: string) {
      if (property === "$transaction")
        return async <T>(callback: (tx: typeof db) => Promise<T>) => callback(db)
